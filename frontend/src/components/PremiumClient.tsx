...
export default function PremiumClient() {
  ...
  return <AppLayout rightPanel={false}>
    <div className="space-y-10 pb-8">
      <section className="relative isolate min-h-[320px] overflow-hidden rounded-[24px] bg-[#100929] px-6 py-9 text-white shadow-[0_25px_65px_rgba(25,14,70,.25)] sm:px-10 lg:px-12">
        <img src="/images/listening-world.png" alt="A magical storybook becoming an audio world" className="absolute inset-0 -z-20 h-full w-full object-cover object-right" />
        <div className="absolute inset-0 -z-10 bg-gradient-to-r from-[#090619] via-[#100929]/95 to-[#100929]/10" />
        <div className="flex min-h-[250px] max-w-xl flex-col justify-center">
          ...
          <h1 className="text-4xl font-black leading-[1.02] tracking-[-.05em] sm:text-5xl lg:text-[56px]">Every story.<br/><span className="text-[#f3d59b]">No limits.</span></h1>
      ...
      <section id="plans" className="scroll-mt-8">
        <div className="mx-auto max-w-2xl text-center"><span className="text-[11px] font-bold uppercase tracking-[.18em] text-primary">Simple pricing</span><h2 className="mt-2 text-2xl font-bold tracking-[-.035em]">Choose your listening plan</h2><p className="mt-2 text-sm text-textMuted">No hidden fees. Change or cancel anytime.</p></div>
      ...
      <section className="grid overflow-hidden rounded-[26px] bg-soft lg:grid-cols-[.9fr_1.1fr]"><div className="flex flex-col justify-center p-7 sm:p-9"><div className="flex gap-1 text-[#f0ad35]">{[1,2,3,4,5].map(i=><Star key={i} size={15} fill="currentColor"/>)}</div><blockquote className="mt-4 text-xl font-extrabold leading-8 tracking-[-.03em]">“EchoTale turns my commute into a new adventure. The narration feels genuinely cinematic.”</blockquote><p className="mt-4 text-sm font-bold">Maya R. <span className="font-normal text-textMuted">— Premium listener</span></p></div><div className="relative min-h-[240px] bg-[#17103d]"><img src="/images/listening-world.png" alt="Immersive audiobook worlds" className="absolute inset-0 h-full w-full object-cover object-right opacity-80"/><div className="absolute inset-0 bg-gradient-to-r from-[#17103d] to-transparent"/><div className="absolute bottom-5 left-5 rounded-xl border border-white/10 bg-white/10 p-3 text-white backdrop-blur"><Headphones size={20} className="text-[#f7ce74]"/><p className="mt-1 text-sm font-bold">Your world. Your pace.</p></div></div></section>

      <section className="mx-auto max-w-3xl"><div className="text-center"><span className="text-[11px] font-bold uppercase tracking-[.18em] text-primary">Questions</span><h2 className="mt-2 text-2xl font-bold tracking-[-.035em]">Good to know</h2></div><div className="mt-6 space-y-2">{faqs.map(([q,a],i)=><button type="button" aria-expanded={openFaq===i} key={q} onClick={()=>setOpenFaq(openFaq===i?-1:i)} className="w-full rounded-[18px] border border-borderSoft bg-white px-5 py-4 text-left shadow-sm"><span className="flex items-center justify-between gap-4 text-sm font-bold">{q}<ChevronDown size={18} className={`shrink-0 text-primary transition ${openFaq===i?"rotate-180":""}`}/></span>{openFaq===i&&<p className="mt-3 pr-8 text-sm leading-6 text-textMuted">{a}</p>}</button>)}</div></section>
    </div>
  </AppLayout>;
}
