...
export function EchoTaleLogo({ compact = false, inverse = false, className }: { compact?: boolean; inverse?: boolean; className?: string }) {
  return (
    <div className={cn("flex items-center gap-3", className)}>
      <span className="relative grid h-11 w-11 shrink-0 place-items-center overflow-hidden rounded-[14px] bg-[#181331] shadow-[0_10px_24px_rgba(8,5,27,.22)] ring-1 ring-white/10">
      ...
      {!compact && <span><span className={cn("block text-[22px] font-black tracking-[-.045em]", inverse ? "text-white" : "text-[#17132a]")}>EchoTale</span><span className={cn("block text-[10px] font-semibold tracking-[.09em]", inverse ? "text-white/45" : "text-[#777181]")}>STORIES THAT STAY</span></span>}
    </div>
  );
}