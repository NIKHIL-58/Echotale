...
export function AppLayout({ children, rightPanel = true }: { children: React.ReactNode; rightPanel?: boolean }) {
  ...
  return <div className="min-h-screen bg-[radial-gradient(circle_at_85%_0%,rgba(126,91,211,.10),transparent_30rem)]"><Sidebar /><main className="min-h-screen px-4 pb-28 pt-4 sm:px-6 lg:ml-[272px] lg:px-9 lg:pt-7"><Suspense fallback={null}><Topbar /></Suspense><div className="mx-auto mt-8 flex max-w-[1600px] gap-7"><section className="min-w-0 flex-1">{children}</section>{rightPanel && <RightPanel />}</div></main><BottomPlayer /><MobileBottomNav /></div>;
}
