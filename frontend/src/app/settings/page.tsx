  ...
  return (
    <AppLayout rightPanel={false}>
      <form onSubmit={save} className="mx-auto max-w-6xl space-y-6">
        <header className="relative isolate overflow-hidden rounded-[28px] bg-[#0d0a22] px-6 py-8 text-white shadow-[0_22px_55px_rgba(24,17,51,.2)] sm:px-8">
          <img src="/premium-story-world.png" alt="" className="absolute inset-0 -z-20 h-full w-full object-cover object-right opacity-20" />
        ...
        {message && <div className={`flex items-center gap-3 rounded-2xl border px-5 py-4 text-sm font-semibold ${error ? "border-red-100 bg-red-50 text-red-700" : "border-emerald-100 bg-emerald-50 text-emerald-700"}`}><CheckCircle2 size={18} />{message}</div>}

        <div className="grid gap-6 lg:grid-cols-[1.25fr_.75fr]">
          <div className="space-y-6">
            <section className="rounded-[26px] border border-borderSoft bg-white p-6 shadow-soft sm:p-7">
              ...
              <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4">{languages.map((item)=><button key={item} type="button" onClick={()=>setLanguage(item)} className={`flex h-12 items-center justify-between rounded-xl border px-4 text-sm font-bold transition ${language===item ? "border-primary bg-soft text-primary shadow-[0_7px_18px_rgba(118,87,211,.12)]" : "border-borderSoft bg-white text-textMuted hover:border-primary/20 hover:text-textMain"}`}>{item}{language===item&&<Check size={16}/>}</button>)}</div>
            </section>
            ...
            <section className="rounded-[26px] border border-borderSoft bg-white p-6 shadow-soft sm:p-7">
              ...
              <input type="range" min={5} max={180} step={5} value={goal} onChange={(event)=>setGoal(Number(event.target.value))} className="mt-7 w-full accent-primary" />
          ...
          <div className="space-y-6">
            <section className="rounded-[26px] border border-borderSoft bg-white p-6 shadow-soft">
            ...
            <Link href="/premium" className="group block overflow-hidden rounded-[26px] bg-gradient-to-br from-[#251b4c] to-[#0d0a22] p-6 text-white shadow-card"><div className="flex items-start justify-between"><span className="grid h-11 w-11 place-items-center rounded-2xl bg-white/10 text-[#e5bd6b]"><Crown size={20} /></span><ChevronRight className="text-white/35 transition group-hover:translate-x-1 group-hover:text-white" size={19}/></div><h2 className="mt-5 font-extrabold">EchoTale Premium</h2><p className="mt-2 text-sm leading-6 text-white/55">Unlock unlimited listening, offline stories, and premium narration.</p><span className="mt-4 inline-flex items-center gap-2 text-xs font-bold text-[#e5bd6b]"><ShieldCheck size={14}/>View your plan</span></Link>
          </div>
        </div>

        <footer className="sticky bottom-4 z-10 flex flex-col gap-3 rounded-[22px] border border-borderSoft bg-white/90 p-4 shadow-[0_16px_45px_rgba(26,18,51,.14)] backdrop-blur-xl sm:flex-row sm:items-center sm:justify-between"><p className="text-sm text-textMuted">Changes are applied to your EchoTale account.</p><button type="submit" disabled={saving} className="inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-primary px-5 text-sm font-bold text-white shadow-[0_9px_22px_rgba(118,87,211,.22)] disabled:opacity-60">{saving?<><Loader2 size={16} className="animate-spin"/>Saving...</>:<><Save size={16}/>Save preferences</>}</button></footer>
      </form>
    </AppLayout>
  );
}
